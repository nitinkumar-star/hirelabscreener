...
def migrate(conn):
    ...
    # Additive migration: dedicated GST billing fields on the client so invoices
    # auto-fill from a single source of truth (bill-to address, state, state code).
    for col, defn in [
        ('bill_address', "TEXT DEFAULT ''"),      # GST billing address (may differ from office address)
        ('bill_state', "TEXT DEFAULT ''"),        # billing state name (e.g. Haryana)
        ('bill_state_code', "TEXT DEFAULT ''"),   # GST state code (e.g. 06) — decides IGST vs CGST/SGST
    ]:
        try:
...
class ClientRepo:
    ...
    def insert(conn, data):
        ...
        conn.execute(
            f'INSERT INTO crm_clients ({cols}) VALUES ({",".join("?" * 21)})',
            (data['company_id'], data['name'], data['industry'], data['website'],
             data['city'], data['state'], data['country'], data['gstin'],
             data['address'], data.get('bill_address', ''), data.get('bill_state', ''),
             data.get('bill_state_code', ''), data['status'], data['owner_user_id'],
             data['notes'], data['name_key'], 1, data['actor'], data['actor'],
             data['now'], data['now']))
...
class ClientService:
    ...
    def create(conn, payload):
        ...
        data = {
            ...
            'bill_address': (payload.get('bill_address') or '').strip(),
            'bill_state': (payload.get('bill_state') or '').strip(),
            'bill_state_code': (payload.get('bill_state_code') or '').strip(),
            'status': status,
